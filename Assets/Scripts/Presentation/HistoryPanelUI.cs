...
public class HistoryPanelUI : MonoBehaviour
{
    ...
    // Bulk-load path for session restore — AddRecord's per-call Rebuild() destroys
    // and recreates every row from scratch, so replaying a whole saved session one
    // record at a time (as GameManager's load path used to) rebuilds the full row
    // list once per saved spin, an O(n^2) cost that's the main reason switching
    // into Roulette got slower the longer a save file's history grew. This adds
    // every record to the list first and rebuilds exactly once at the end.
    ...
    void Rebuild(bool animateNewest)
    {
        ...
        for (int i = 0; i < records.Count; i++)
        {
            ...
            MakeRowText(rowGO.transform, UIFactory.FormatMoney(rec.TotalStaked), 2);
            MakeRowText(rowGO.transform, $"{sign}{UIFactory.FormatMoney(rec.NetChange)}", 3, netColor);
