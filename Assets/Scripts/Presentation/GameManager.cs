...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (UseBallWheel)
        {
            var wheelGO = new GameObject("CasinoWheel");
            var ballWheel = wheelGO.AddComponent<RouletteBallWheel>();
            ballWheel.Build();
            wheel = ballWheel;
        }
        else
        {
            ...
            wheel = new ClassicRouletteWheel(builder, wheelAnimator);
        }
    ...
    void SetupCamera()
    {
        ...
        // The ball wheel is viewed at a casino-table tilt so the ball's run round the track reads;
        // the needle wheel keeps its near top-down view.
        camGO.transform.position = UseBallWheel ? new Vector3(0, 9.4f, -4.6f) : new Vector3(0, 10.5f, -0.6f);
    ...
    void SetupUI()
    {
        ...
        juiceManager.Build(canvasGO.transform, cameraTransform, wheel.Center + Vector3.up * 1f, keyLight);
        ...
        hud.Build(canvasGO.transform, bankroll,
            addAmount =>
            {
                ...
                SaveSystem.Save(bankroll, nextSpinIndex, sessionRecords, bettingController.OnTableTotal());
            },
            resetAmount =>
            {
        ...
        bettingController.Build(canvasGO.transform, bankroll, chipSelector, generator, belt, wheel, pastSpinsStrip,
        ...
        // Hover names for the felt's split / street / corner / six-line spots; built last so it draws on top
        TooltipUI.Create(canvasGO.transform);
    ...
    // immediately) — catches anything else that might end the process. Leaving the
    // table (quit or switching games): chips not yet spun go back to the wallet; a spin
    // mid-reveal is already decided, so it's paid and kept in the history (the hot/cold
    // tracker needs every spin).
    ...
    void LeaveTable()
    {
        ...
        var unfinished = bettingController.RefundTableBets();
        if (unfinished != null)
        {
            sessionRecords.Add(unfinished);
            nextSpinIndex = unfinished.SpinIndex + 1;
        }
        SaveSystem.Save(bankroll, nextSpinIndex, sessionRecords, bettingController.OnTableTotal());
    }
}
