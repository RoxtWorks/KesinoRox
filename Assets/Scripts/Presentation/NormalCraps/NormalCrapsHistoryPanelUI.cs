...
public class NormalCrapsHistoryPanelUI : MonoBehaviour
{
    ...
    readonly List<NormalCrapsRoundRecord> records = new List<NormalCrapsRoundRecord>();
    ...
    {
        UIFactory.MakePanel(canvas, "NCHistoryPanelBg", anchoredPos, size, UIFactory.PanelDark);
        UIFactory.MakeSectionHeader(canvas, "History", anchoredPos + new Vector2(0, size.y / 2f - 20f), new Vector2(size.x - 20, 20));

        var headerRow = new GameObject("NCHistoryHeaderRow");
        ...
        var scrollGO = new GameObject("NCHistoryScroll");
    ...
    public void AddRecord(NormalCrapsRoundRecord record)
    {
        records.Add(record);
